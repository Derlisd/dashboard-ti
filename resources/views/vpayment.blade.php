...
<meta charset="utf-8">
    ...
    <div class="container">
        <div class="form-row mb-2">
            <div class="col-10">
                <textarea cols="30" rows="1" class="form-control" name="query" id="query"> SELECT * FROM vPayments limit 100;</textarea>
            </div>
...
@section('js')
    ...
    <script>
        ...
        const loadTable = async () =>{
            showLoading()
            let items = await getVpayments();
            hideLoading()
            $('#tablaDatos').DataTable().destroy()
            buildTableVpayment(items)
            
        }
        const getVpayments = async  ()=>{
            let query = $('#query').val();
        ...
        const buildTableVpayment = (vpayments) =>{
            TABLE = $('#tablaDatos').DataTable({
                ...
                columns: [
                    ...
                    { data: 'ID', name: 'ID' },
                    { data: 'date', name: 'date' },
                    { data: 'payoutDate', name: 'payoutDate' },
                    { data: 'depositedDate', name: 'depositedDate' },
                    { data: 'amount', name: 'amount' },
                    { data: 'payoutAmount', name: 'payoutAmount' },
                    { data: 'comission', name: 'comission' },
                    { data: 'tax', name: 'tax' },
                    { data: 'deposited', name: 'deposited' },
                    { data: 'orderNo', name: 'orderNo' },
                    { data: 'authCode', name: 'authCode' },
                    { data: 'operationNo', name: 'operationNo' },
                    { data: 'inBank', name: 'inBank' },
        ...
        const validateOperacion= async (query) =>{
            ...
            try {
                ...
                method: 'POST',
                data:{'campo_query' : query, 'modulo':'vpayment'},
        ...
        $('#tablaDatos').on('click', '.editar', function() {
            ...
            DATA_UPDATE['ID'] = row.cell(tr.find('td:nth-child(2)')).data(); // Obtiene el valor de la segunda columna
